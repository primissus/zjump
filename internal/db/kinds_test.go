...
// different name stays separate (§3).
...
// TestPutAliasAndTouchAlias covers alias creation, in-place target replacement
// (rank/last_accessed preserved), and the on-use bump (§3, §5.2, §5.4).
func TestPutAliasAndTouchAlias(t *testing.T) {
	database, _ := OpenDir(t.TempDir())
	database.PutAlias("work", "/a", 100)
	a := database.FindAlias("work")
	if a == nil || a.Path != "/a" || a.Rank != 1.0 || a.LastAccessed != 100 {
		t.Fatalf("new alias = %+v, want /a rank 1 last 100", a)
	}

	database.PutAlias("work", "/b", 999) // replace target, preserve rank/last
	a = database.FindAlias("work")
	if a.Path != "/b" {
		t.Errorf("replaced target = %q, want /b", a.Path)
	}
	if a.Rank != 1.0 || a.LastAccessed != 100 {
		t.Errorf("replace changed rank/last: %+v, want rank 1 last 100 preserved", a)
	}

	if !database.TouchAlias("work", 500) {
		t.Fatal("TouchAlias returned false for existing alias")
	}
	a = database.FindAlias("work")
	if a.Rank != 2.0 || a.LastAccessed != 500 {
		t.Errorf("after touch = rank %v last %d, want rank 2 last 500", a.Rank, a.LastAccessed)
	}
	if database.TouchAlias("missing", 1) {
		t.Error("TouchAlias returned true for a missing alias")
	}
}

// TestRemoveAndFindPathIgnoreAliases: Remove and the path mutators never match an
// alias that happens to share a path with a real entry (§3).
func TestRemoveAndFindPathIgnoreAliases(t *testing.T) {
	database, _ := OpenDir(t.TempDir())
	database.AddUpdate("/shared", 1.0, 100, KindDir)
	database.PutAlias("s", "/shared", 100) // alias targeting the same path

	// Remove targets the dir entry, leaving the alias.
	if !database.Remove("/shared") {
		t.Fatal("Remove failed to find the dir entry")
	}
	if database.FindAlias("s") == nil {
		t.Error("Remove deleted the alias sharing the path")
	}
	if database.findPath("/shared") != nil {
		t.Error("dir entry should be gone")
	}

	// Remove on a path that now only exists as an alias target finds nothing.
	if database.Remove("/shared") {
		t.Error("Remove matched an alias by its target path")
	}
	if database.FindAlias("s") == nil {
		t.Error("alias must survive a Remove of its target path")
	}
}
