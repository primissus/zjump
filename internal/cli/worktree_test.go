...
func withWorktreeLister(t *testing.T, fn func(repo string) ([]gitx.Worktree, error)) {
	t.Helper()
	orig := worktreeLister
	worktreeLister = fn
...
func TestWorktreeDedupAcrossRepos(t *testing.T) {
	...
	withWorktreeLister(t, func(repo string) ([]gitx.Worktree, error) {
		switch repo {
		case "/repoA":
			return []gitx.Worktree{{Path: "/wt/shared", Branch: "main"}, {Path: "/wt/a", Branch: "x"}}, nil
		case "/repoB":
			return []gitx.Worktree{{Path: "/wt/shared", Branch: "main"}, {Path: "/wt/b", Branch: "y"}}, nil
		}
		return nil, nil
	})
...
func TestWorktreeRepoCapAt50(t *testing.T) {
	...
	withWorktreeLister(t, func(repo string) ([]gitx.Worktree, error) {
		return []gitx.Worktree{{Path: repo + "/wt", Branch: "main"}}, nil
	})
...
func TestWorktreeGitFailureSkipsRepoSilently(t *testing.T) {
	...
	withWorktreeLister(t, func(repo string) ([]gitx.Worktree, error) {
		...
		return []gitx.Worktree{{Path: "/wt/good", Branch: "main"}}, nil
	})
...
func TestWorktreeFirstAndExclude(t *testing.T) {
	...
	withWorktreeLister(t, func(repo string) ([]gitx.Worktree, error) {
		return []gitx.Worktree{{Path: "/wt/only", Branch: "main"}}, nil
	})
