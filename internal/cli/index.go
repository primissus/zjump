...
const indexHelp = `Usage: zjump index [--max-depth N] [--score S] <ROOT>...

Bulk-scan directories for git repositories and add each as a
typed repository entry to the frecency database. zjump extension.

Flags:
    --max-depth N    Maximum directory depth to descend (root is depth 0;
                     default 3)
    -s, --score S    Rank to add for each indexed repository (default 1.0)
`
...
func runIndex(args []string) error {
	...
	var score float64
	fs.Float64Var(&score, "s", 1.0, "rank to add for each indexed repository")
	fs.Float64Var(&score, "score", 1.0, "rank to add for each indexed repository")

	roots, err := parseArgs(fs, args)
	if err != nil {
		if err == flag.ErrHelp {
			printCmdHelp(os.Stdout, "index", indexHelp)
			return nil
		}
		return err
	}
